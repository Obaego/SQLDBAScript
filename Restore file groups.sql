...
(
    NAME = ColdData,
    FILENAME = 'G:\BACKUP FOLDER\ColdData.ndf',
    SIZE = 10MB,
    MAXSIZE = 100MB,
    FILEGROWTH = 10MB
)
...
BACKUP DATABASE FGRestoreDB
    TO DISK = 'G:\BACKUP FOLDER\FGRestoreDB.bak'
    WITH FORMAT;
GO
...
RESTORE DATABASE FGRestoreDB
   FROM DISK = 'G:\BACKUP FOLDER\FGRestoreDB.bak'
   WITH RECOVERY
GO
...
BACKUP DATABASE FGRestoreDB
   ...
   TO DISK = 'G:\BACKUP FOLDER\ColdData.bak'
GO
...
RESTORE DATABASE FGRestoreDB
   FILE = 'ColdData',
   FILEGROUP = 'ColdStorageFG'
   FROM DISK = 'G:\BACKUP FOLDER\ColdData.bak'
   WITH RECOVERY
GO

-- need a tail log backup first
BACKUP LOG FGRestoreDB
   TO DISK = 'G:\BACKUP FOLDER\Taillog.bak'
   WITH NO_TRUNCATE;
GO
...
RESTORE LOG FGRestoreDB
   FROM DISK = 'G:\BACKUP FOLDER\TailLog.bak'
   WITH RECOVERY
GO
